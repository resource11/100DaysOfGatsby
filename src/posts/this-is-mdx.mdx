...
<h2>Metadata title is {metadata.title}</h2>
<p>Metadata author is {metadata.author}</p>
---

## Can you write in MDX and use a GraphQL page query in MDX?

Yes you can! This page is an MDX file that is exporting metadata. Not sure if we want to use the frontmatter GraphQL query in this file though. This page uses a default export, so it won't take on the default layout set by the gatsby-config file.
